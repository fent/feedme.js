...
var childTag = function(parser, parent, node, parentcb) {
  ...
  var closef = function(name) {
    ...
    if (gottext !== '') {
      ...
      // if there is no childrern in this tag,
      // or the only child is type,
      // then make the whole tag the text
      if (isEmpty(obj) ||
         (Object.keys(obj).length === 1 && obj.type !== undefined)) {
        obj = gottext;
      } else {
        obj.text = gottext;
      }
...
module.exports = function(buffer) {
  var parser = sax.createStream(false, { lowercasetags: true, trim: false });
