(function() {
  ...
  childTag = function(parser, parent, node, parentcb) {
    ...
    openf = function(node) {
      if (!gottext) parser.removeListener('text', textf);
  ...
  trimIndent = function(str) {
    var endindent, i, indent, rs, s, split, _len;
    rs = TRIM_REGEXP.exec(str);
    if (rs !== null) {
      split = str.split('\n');
      indent = rs[1] || rs[2] || rs[3];
      endindent = rs[0].substring(0, rs[0].length - indent.length);
      if (split.shift() !== '' || split.pop() !== endindent) return str;
      for (i = 0, _len = split.length; i < _len; i++) {
        s = split[i];
        if (s.indexOf(rs[0]) !== 0) return str;
